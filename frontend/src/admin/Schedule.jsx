      ...
      <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
        ...
        {activity.status === 'pending' && !isMissed(activity) && (
          <>
            ...
            <button
              onClick={() => onCancel(activity.schedule_id)}
              className="flex-1 px-4 py-1.5 text-sm border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition sm:flex-none"
            >
              Cancel
            </button>
          </>
...
function Schedule() {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      setActivities(scheduleRes.data || []);
      setUsers((usersRes.data || []).filter((user) => user.status === 'active'));
    } catch (err) {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axiosClient.patch(`/schedule/${id}/status`, { status });
      fetchActivities();
    } catch (err) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <button
          ...
          className="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition w-full sm:w-auto"
        >
