...
class ReferralNeedsService {
  ...
  bool _isChildSevere(Child c) =>
      ...
      (c.nutritionStatus == 'Severely Underweight' ||
          c.stuntingStatus == 'Severely Stunted' ||
          c.wastingStatus == 'SAM');

  bool _hasOpenReferral(String beneficiaryId) => _referralRepo
      .getForBeneficiary(beneficiaryId)
      .any((r) => r.status == 'Pending' || r.status == 'In Progress');

  List<Child> getChildrenNeedingReferral(String barangay) => _childRepo
      .getByBarangay(barangay)
      .where((c) => _isChildSevere(c) && !_hasOpenReferral(c.id))
      .toList();
  ...
  List<Mother> getMothersNeedingReferral(String barangay) => _motherRepo
      ...
      .where(
        (m) =>
            m.barangay == barangay &&
            m.isActive &&
            m.riskStatus == 'At-risk' &&
            !_hasOpenReferral(m.id),
      )
      .toList();
}
