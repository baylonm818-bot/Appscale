...
class InfoTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final MotherVisit? latestConcernVisit = mother.riskStatus == 'At-risk'
        ? visits.firstWhere(
            (v) => v.present && v.observation == 'Signs of concern',
            orElse: () => visits.first,
          )
        : null;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Column(
              children: [
                ...
                DetailRow(
                  ...
                  label: 'Age',
                  value: mother.ageLabel,
                ),
                const SizedBox(height: AppSpacing.md),
                DetailRow(
                  icon: Icons.event_outlined,
                  label: 'Birth Date',
                  value: _formatDate(mother.birthDate),
                ),
                const SizedBox(height: AppSpacing.md),
                DetailRow(
                  icon: Icons.call_outlined,
                  label: 'Contact no.',
                  value: mother.contactNo.isEmpty
                      ? 'Not provided'
                      : mother.contactNo,
                ),
                const SizedBox(height: AppSpacing.md),
                ...
                DetailRow(
                  ...
                  label: 'IP Group',
                  value: mother.belongsToIpGroup ? 'Yes' : 'None',
                ),
                const SizedBox(height: AppSpacing.md),
                DetailRow(
                  icon: Icons.accessibility_new_outlined,
                  label: 'Disability',
                  value: mother.disability,
                ),
              ],
            ),
          ),
          ...
          Text(
            'Sourced from the latest counseling visit — update it by logging a new visit, not here.',
          ...
          SectionCard(
            ...
            child: Row(
              children: [
                ...
                _StatBlock(
                  label: 'Last visit',
                  value: visits.isEmpty ? '—' : _formatDate(visits.first.date),
                ),
              ],
            ),
          ),
          if (mother.riskStatus == 'At-risk' && latestConcernVisit != null) ...[
            ...
            Padding(
              ...
              child: Text(
                'Automatically clears once a future visit shows "Appears well".',
                style: AppTextStyles.body.copyWith(
                  fontSize: 11,
                  color: AppColors.textMuted,
                ),
